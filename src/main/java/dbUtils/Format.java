...
public class Format {
    ...
    // formats decimal numbers (ex: 3.5, 170.5, 20.0)
    public static String fmtDecimal(Object obj) {
        ...
        try {
            ...
            if (obj instanceof BigDecimal) {
                return decFormat.format((BigDecimal) obj);
            }
            if (obj instanceof Double) {
                return decFormat.format((Double) obj);
            }
            if (obj instanceof Integer) {
                return decFormat.format((Integer) obj);
            }

            return decFormat.format(Double.parseDouble(obj.toString()));

        } catch (Exception e) {
            return "bad Decimal in FormatUtils:" + obj.toString() + 
                ". Error:" + e.getMessage();
        }
    } // formatDecimal
